...
class MNIST(data.Dataset):
    ...
    def __init__(
        ...
        if right_noise_p > 0:
            self.multi_training_file = f"multi_training_rnoise_{right_noise_p}.pt"
            self.multi_test_file = f"multi_test_rnoise_{right_noise_p}.pt"

        if both_noise_min is not None:
            self.multi_training_file = f"multi_training_bnoise_{both_noise_min}.pt"
            self.multi_test_file = f"multi_test_bnoise_{both_noise_min}.pt"

        # amount of salt and pepper noise in the right image
        self.right_noise_p = right_noise_p
        self.both_noise_min = both_noise_min
    ...
    def download(self):
        ...
        mnist_exists = self._check_exists()

        if mnist_exists and self._check_multi_exists():
            return
...
def read_image_file(path, right_noise_p=0, both_noise_min=None):
    with open(path, "rb") as f:
        data = f.read()
        ...
        for left in range(length):
            ...
            for j, right in enumerate(chosen_ones):
                ...
                left_noise_p = 0
                if both_noise_min is not None:
                    left_noise_p = np.random.uniform(both_noise_min, 1)
                    right_noise_p = np.random.uniform(both_noise_min, 1)

                if left_noise_p > 0:
                    left_noise_mask = (
                        np.random.uniform(0, 1, size=(28, 28)) > left_noise_p
                    ).astype(int)
                    lim = left_noise_mask * lim

                if right_noise_p > 0:
                    right_noise_mask = (
                        np.random.uniform(0, 1, size=(28, 28)) > right_noise_p
                    ).astype(int)
                    rim = right_noise_mask * rim

                new_im = np.zeros((36, 36))
                new_im[0:28, 0:28] = lim
                new_im[6:34, 6:34] = rim
                new_im[6:28, 6:28] = np.maximum(lim[6:28, 6:28], rim[0:22, 0:22])
                # multi_data_im = transf.resize(new_im, (28, 28), interp="nearest")
                multi_data_im = transf.resize(new_im, (28, 28))
...
if __name__ == "__main__":
    ...
    loader = torch.utils.data.DataLoader(
        dst, batch_size=10, shuffle=True, num_workers=4
    )
    for dat in loader:
